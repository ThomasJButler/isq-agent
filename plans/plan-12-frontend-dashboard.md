...
Run these from the **main checkout** (`/Users/tombutler/Repos/isq-agent`) once this plan + the frontend `IMPLEMENTATION_PLAN.md` (§7) are committed to `main`:

```bash
# 1. From the main checkout, create the frontend branch in a SEPARATE worktree dir
git -C /Users/tombutler/Repos/isq-agent worktree add -b feature/frontend-nextjs ../isq-agent-frontend main
