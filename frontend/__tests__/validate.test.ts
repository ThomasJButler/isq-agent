...
// Slice 5 — upload validation. Accepts a `.pdf`/`.docx`/`.xlsx` (the backend's
// _SUPPORTED_SUFFIXES and the UI's "PDF, DOCX or XLSX" promise), checks the TYPE
// first, then the SIZE, and surfaces these exact messages. These tests lock that
// contract (copy + boundary + precedence) so the validator and the Dropzone that
// shows its messages can never drift apart.
...
describe("validateUpload — accepted types", () => {
  ...
  it("accepts a .docx", () => {
    expect(validateUpload(file("questionnaire.docx"))).toEqual({ ok: true });
  });

  it("matches the extension case-insensitively", () => {
    expect(validateUpload(file("REPORT.PDF"))).toEqual({ ok: true });
    expect(validateUpload(file("Book.XLSX"))).toEqual({ ok: true });
    expect(validateUpload(file("Answers.DOCX"))).toEqual({ ok: true });
  });
});

describe("validateUpload — rejected types", () => {
  it("rejects a .csv with the type message", () => {
    expect(validateUpload(file("answers.csv"))).toEqual({
      ok: false,
      reason: "type",
      message: "We couldn't read this file. Try a PDF, DOCX or XLSX.",
    });
  });
...
describe("validateUpload — size ceiling", () => {
  it("exposes a 10 MiB ceiling and the accepted extensions", () => {
    ...
    expect(ACCEPTED_EXTENSIONS).toEqual([".pdf", ".docx", ".xlsx"]);
  });
...
describe("validateUpload — check order", () => {
  ...
  it("reports the type error first when a file is both wrong type and oversized", () => {
    expect(validateUpload(file("huge.csv", MAX_UPLOAD_BYTES + 1))).toMatchObject({
      ok: false,
      reason: "type",
    });
  });
